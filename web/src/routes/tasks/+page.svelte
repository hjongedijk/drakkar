<script lang="ts">
  ...
  const tasks: TaskDef[] = [
    ...
    { id: 'reset_orphaned_available', label: 'Reset Orphaned Available Items', description: 'Reset available items with no symlink and no recoverable source back to pending so they are re-searched and re-downloaded.', group: 'Publishing', interval: 'Manual', manual: true, run: async () => { const r = await api.resetOrphanedAvailableItems(); return `found ${r.found}, reset ${r.reset}, failed ${r.failed}`; } },
