<script lang="ts">
  ...
  const tasks: TaskDef[] = [
    ...
    { id: 'health_check', label: 'Run Health Check', description: 'Verify published symlinks still point to valid VFS targets.', group: 'Maintenance', interval: '15m', manual: true, run: async () => { const r = await api.runHealthCheck(); return `checked ${r.checked}, healthy ${r.healthy}`; } },
    { id: 'nzb_health_check', label: 'Deep NZB Article Check', description: 'Probe first/last NNTP articles for available items and reset missing-article or sample-only publications for re-queue.', group: 'Maintenance', interval: '168h', manual: false, run: async () => '' },
